...
training_data=read.csv('Total_Data_reduced_features.csv')
testing_data=read.csv('MBA_data_test_reduced_features.csv')
...
############## Only NN ###################
    ...
    actual_cl_total_data_CART<-training_data$Placement
    n <- names(training_data)
    
    

    f1 <- as.formula(paste("Placement ~", paste(n[!n %in% "Placement"], collapse = " + ")))
    #tree_1 <-  rpart(f1, data = data, control = rpart.control(cp =0.010))
    #tree_1 <-  rpart(f1, data = data, control = rpart.control(minsplit = 20))
    tree_1 <-  rpart(f1, data = training_data, control = rpart.control(minsplit = 20))

    plot(tree_1)
    text(tree_1, use.n=TRUE, all=TRUE, cex=.8)

    #predicted_cl_total_data_CART<-as.integer(round(predict(tree_1,data)))
    predicted_cl_total_data_CART<-as.integer(round(predict(tree_1,training_data)))
    ...
    tree_11 <-  rpart(f11, data = training_data, control = rpart.control(cp =0.10))
...
#     test_data <- scaled[test_row_number,]
    ...
    ############  scaled individual training and testing data sets ##################
     ...
     maxs1 <- apply(testing_data, 2, max)
     mins1 <- apply(testing_data, 2, min)
     testing_scaled <- as.data.frame(scale(testing_data, center = mins1, scale = maxs1 - mins1))
    ...
    train_data$Placement=predicted_cl_total_data_CART
    #######

    ###actual class label of test data  ###
     
     test.r= (test_data$Placement)*(max(test_data$Placement)-min(test_data$Placement))+min(test_data$Placement)
    ...
    ########  predicted class label of test data after nn #########
    pr.nn1_1=pr.nn1$net.result*(max(test_data$Placement)-min(test_data$Placement))+min(test_data$Placement)
    ##########
    ...
    #table(data[201:236,]$Placement, as.integer(round(pr.nn1$net.resul[,1])))
    #confusion_matrix_cart_nn<-table(test.r_cart, as.integer(round(pr.nn1$net.resul[,1])))
    confusion_matrix_cart_nn<-table(predicted_cl_test_data_CART, as.integer(round(pr.nn1$net.resul[,1])))
